...
uint8_t RandomSeedSeed = random(1,255);
void TitleScreen() {
  ...
  while (true) {
    if ( Game.ButtonOffCD() ) {
      ...
      if ( ( input & UP_BUTTON ) || ( input & DOWN_BUTTON ) ) SoundOn = !SoundOn;
    ...
    display.clear();
    display.drawSlowXYBitmap(3,3,TitleText,48,25,1);
    display.drawSlowXYBitmap(58,10,TitleGfx,72,57,1);
    //display.setCursor(0,48); display.print(F("Level: ")); display.print(RandomSeedSeed);
    display.setCursor(0,56); display.print(F("SFX: "));
    RandomSeedSeed = random (1,255); //Always Random
    if ( SoundOn ) display.print(F("On"));
    else display.print(F("Off"));
    display.display();
  }
  if (RandomSeedSeed =1) RandomSeedSeed = random (1,255);
  random( RandomSeedSeed );
